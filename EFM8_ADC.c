...
void InitTimer0(void){
    ...
    CKCON0 |= 0x08; // Use SYSCLK for Timer 0
...
float calculatePhaseShift(unsigned int period_ticks, unsigned int delta_ticks) {
    ...
    float period_time = period_ticks * time_per_tick;  // Convert period to seconds
...
void main (void)
{
	...
	while(1)
	{
		...
		while (ZERO_CROSS_PIN == 0);
			...
			period_seconds = (float)period * (12.0 / SYSCLK);
			frequency = 1.0 / period_seconds;

			printf("\nfrequency: %f hz\n", frequency);
			
			WaitQuarterPeriod(period);
			ADC_at_Pin(REF_peak); //p2.2
			peak = Volts_at_Pin(REF_peak); //p2.2
			printf("\npeak (ref): %f v\n", peak);
			
			delta_ticks = phaseDif();
			phase_shift = calculatePhaseShift(period, delta_ticks);
			printf("\nphase shift: %f °\n", phase_shift);
		//}
		...
		phase_shift = calculatePhaseShift(period, phaseDif());
